  ...
  describe('extractJobDescription', () => {
    ...
    it('throws after max retries exhausted', async () => {
      vi.mocked(anthropicService.callClaude)
        ...
        .mockResolvedValueOnce('bad');
      await expect(
        extractJobDescription('fail-desc-' + Date.now()),
      ).rejects.toThrow('Failed to extract job description after 3 attempts');
    });
  });
  ...
  describe('analyzeAndSaveJob', () => {
    it('extracts, scores, creates job, and updates with fit score', async () => {
      ...
      const updatedJob = {
        ...mockJob,
        fit_score: 85,
        fit_explanation: validFitScore.explanation,
      };
      vi.mocked(jobsRepo.updateJob).mockResolvedValueOnce(updatedJob);

      const result = await analyzeAndSaveJob(
        'analyze-desc-' + Date.now(),
        userId,
      );
