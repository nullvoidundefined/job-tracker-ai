...
vi.mock('app/handlers/jobs/jobs.js', () => ({
  listJobs: (_: express.Request, res: express.Response) =>
    res.status(200).json({ ok: true }),
  createJob: (_: express.Request, res: express.Response) =>
    res.status(201).json({ ok: true }),
  getJob: (_: express.Request, res: express.Response) =>
    res.status(200).json({ ok: true }),
  updateJob: (_: express.Request, res: express.Response) =>
    res.status(200).json({ ok: true }),
  deleteJob: (_: express.Request, res: express.Response) =>
    res.status(204).send(),
}));
vi.mock('app/handlers/profile/profile.js', () => ({
  getProfile: (_: express.Request, res: express.Response) =>
    res.status(200).json({ ok: true }),
  updateProfile: (_: express.Request, res: express.Response) =>
    res.status(200).json({ ok: true }),
}));
