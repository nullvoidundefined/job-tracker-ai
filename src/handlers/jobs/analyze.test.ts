...
describe("POST /jobs/analyze", () => {
  ...
  it("returns 400 when raw_description is too short", async () => {
    const res = await request(app).post("/jobs/analyze").send({ raw_description: "short" });
    expect(res.status).toBe(400);
    expect(res.body.error.message).toContain("at least 20 characters");
  });

  it("returns 502 when AI analysis fails", async () => {
    vi.mocked(analyzerService.analyzeAndSaveJob).mockRejectedValueOnce(new Error("API error"));
    const res = await request(app)
      .post("/jobs/analyze")
      .send({ raw_description: "A long enough job description for testing purposes here" });
    expect(res.status).toBe(502);
    expect(res.body.error.message).toBe("AI analysis failed. Please try again.");
  });
});
