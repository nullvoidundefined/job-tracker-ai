...
import { createJobSchema, jobSchema, updateJobSchema } from "app/schemas/job.js";
...
describe("createJobSchema", () => {
  ...
  it("rejects an invalid status value", () => {
    expect(createJobSchema.safeParse({ status: "unknown" }).success).toBe(false);
  });

  it("rejects title longer than 255 characters", () => {
    expect(createJobSchema.safeParse({ title: "a".repeat(256) }).success).toBe(false);
  });
});
...
describe("updateJobSchema", () => {
  it("is identical to createJobSchema — all fields optional", () => {
    ...
    expect(updateJobSchema.safeParse({ company: "New Co", status: "interviewing" }).success).toBe(
      true,
    );
  });
});
